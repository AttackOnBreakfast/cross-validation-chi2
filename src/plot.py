...
def plot_results(sample, max_degree, chi2_A, chi2_B, chi2_A_theory, chi2_B_theory):
    x_sample, y_sample = sample
    truth_x = np.linspace(0, 1, 500)
    fit_poly = np.poly1d(np.polyfit(x_sample, y_sample, deg=max_degree))
    ...
    # Left: example fit
    plt.subplot(1, 2, 1)
    plt.scatter(x_sample, y_sample, s=10, color='gray', alpha=0.5, label='Data A (Sample)')
    plt.plot(truth_x, f_truth(truth_x), 'k--', lw=2, label='Truth Function')
    plt.plot(truth_x, fit_poly(truth_x), 'r-', lw=2, label=f'Fit (deg={max_degree})')
    plt.title(f"Fit to Sample A with Polynomial Degree {max_degree}")
    plt.xlabel("x")
    plt.ylabel("y")
    plt.legend()
    plt.grid(True)
    ...
    plt.scatter(degrees, chi2_A, color='blue', s=20)
    plt.scatter(degrees, chi2_B, color='red', s=20)
    plt.xlabel("Model Complexity (Polynomial Degree)")
    plt.ylabel(r'$\chi^2$ Value (log scale)')
    plt.title("Average Chi-squared over Seeds")
    plt.yscale('log')
    plt.legend()
